...
public class DbRepoUser implements Repo<Integer, User> {
    ...
    private int paging;
    private int limit; // Numărul de înregistrări returnate per pagină
    /**
    ...
    public DbRepoUser(String url, String username, String password) {
        ...
        this.limit = 3;
        this.paging = -this.limit;
    }

    public void setPagination(int paging) {
        this.paging = -paging;
        this.limit = paging;
    }
    ...
    public Collection<User> findAll() {
        ...
        int recordsNumber = findRecordsNumber();

        if (paging >= recordsNumber - limit) {
            paging = -limit;
        }
        System.out.println("paging is"+paging +"limit is"+limit);
        paging += limit;
        String sql = "SELECT * from users order by id asc limit " + limit + " offset " + paging;
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement(sql);
